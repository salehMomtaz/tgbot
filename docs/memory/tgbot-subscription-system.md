...
## 6) WebApp / Mini App (admin + user portal) — https://tgbot.avistel.ir
...
- **Nginx**: wildcard `*.avistel.ir` cert at `/etc/letsencrypt/live/avistel.ir/{fullchain,privkey}.pem` (valid `tgbot.avistel.ir` via `*.avistel.ir`). `deploy/tgbot.avistel.ir.conf` proxies `https://tgbot.avistel.ir → 127.0.0.1:8080` (FastAPI). HTTP 80 redirects to https (except `/.well-known/acme-challenge`). `install.sh` now auto-installs the vhost when `nginx` + wildcard cert exist (best-effort, never hard-fails; `nginx -t && reload`). `.env` `DOMAIN` set to `https://tgbot.avistel.ir` so streaming links are https.
- **FastAPI mount** (`modules/subscription/webapp.py:mount`): mounts on the existing port-8080 app (already shared with stream_handler).
  - `GET /admin/subscription` → `HTML_ADMIN` panel: toggle enabled/free, multi-channel textarea, tier table, active subs list. Auth: `X-Admin-Token == HMAC(BOT_TOKEN, "admin-sub")[0:16]` (`/admin_token` command) OR Telegram `initData` where `user.id == SYSTEM_CREATOR_ID` (HMAC-SHA256 via `WebAppData` key, per docs). GET `/admin/subscription/api` now **requires admin auth** (previously open); returns settings/tiers/active.
  - `GET /app` → `HTML_USER` portal: any Telegram user via WebApp (valid `initData` required). Shows tier badge, until, quota `remaining/limit`, used today, 7-day history, upgrade cards. “Buy” currently routes to bot (`/subscription`) — invoice still via bot chat (WebApp can't call `sendInvoice` directly).
...
- Telegram side: Bot can expose WebApp via `MenuButton` or inline keyboard `web_app={"url":"https://tgbot.avistel.ir/app"}` for users and `.../admin/subscription` for admin; both work without extra backend because TLS is at nginx.
...
See also: `config.py` (SUB_*), `docs/UBUNTU_VPS_SETUP.md` (§ subscription), `blueprint.md` Phase 21, `deploy/tgbot.avistel.ir.conf`.
