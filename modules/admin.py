...
def register_admin_handlers(app: Client):
    ...
    async def admin_input_handler(client: Client, message: Message):
        ...
        if not is_valid_telegram_id(input_text):
            await message.reply_text("❌ Error: Invalid Telegram ID. Please input digits only (between 5 and 11 numbers).")
            return
        ...
        if "user you want to authorize" in reply_text:
            ...
            if target_id not in db["authorized"]:
                await message.reply_text(f"❌ Error: User ID `{target_id}` is not currently authorized.")
                return
            ...
            if target_id not in db["blacklisted"]:
                await message.reply_text(f"❌ Error: User ID `{target_id}` is not found in the blacklist.")
                return
