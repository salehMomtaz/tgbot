...
async def _render_direct_menu(callback_query: CallbackQuery):
    ...
    try:
        await callback_query.message.edit_text(
            ...
            f"• Linked account: **{direct_forward.pairing_status('x', state)}**\n"
            f"• Cookies: {x_cookie_status}\n"
            f"• X Chat PIN: {pin_status}\n"
            f"• Method: **self-DM only** — message the account in `xcookies.txt` "
            f"from itself (Message Yourself). DMs from other X accounts are not "
            f"relayed yet.\n\n"
            f"**TikTok**\n"
