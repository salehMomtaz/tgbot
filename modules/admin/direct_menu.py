...
async def _render_direct_menu(callback_query: CallbackQuery):
    ...
    try:
        await callback_query.message.edit_text(
            ...
            f"• Linked: **{direct_forward.x_link_status(state)}**\n"
            f"• Cookies: {x_cookie_status}\n"
            f"• X Chat PIN: {pin_status}\n"
            f"• Method: **self-DM** (Message Yourself) always relays; tap "
            f"**🔗 Link X account** and send the code from ANY other X account "
            f"to the bot account to relay that account's DMs too (XChat E2EE "
            f"included).\n\n"
            f"**TikTok**\n"
